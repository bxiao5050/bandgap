    ...
    getApps: function() {
      httpRequest.getApps(data => {
        if (data.state == "successed") {
          ...
          if (this.$store.state.basestore.nowmenu.menu_url == "launchReport") this.$store.dispatch("initApps", apps);
          else if (this.$store.state.basestore.nowmenu.menu_url == "dataRepair") this.$store.dispatch("initDataRepairApps", apps);
