<template>
  <section class="sideBar" :class="{'show':showMenu}">
    ...
    <section class="list-ul px1-t">
      <div class="item" v-for="item in menus" @click="selectMenu(item)">
        <i class="iconfont">&#xe63d;</i>{{item.menu_name}}</div>
      <!--<router-link class="item" :to="{'name':'index'}"><i class="iconfont">&#xe603;</i>五力模型</router-link>-->
    </section>

    <section class="list-ul px1-t">
      <div class="item" @click="clearCatch">
        <i class="iconfont">&#xe699;</i>清空缓存</div>
      <!--<v-touch tag="div" class="item"  v-on:tap="userSet"><i class="iconfont">&#xe690;</i>帐号设置</v-touch>-->
      <div class="item" @click="about">
        <i class="iconfont">&#xe6e5;</i>关于</div>
    </section>
  </section>
